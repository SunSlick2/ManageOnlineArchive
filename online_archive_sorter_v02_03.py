...
class OnlineArchiveSorter:
    ...
    def __init__(self):
        self.config = self._load_config() [cite: 1]
        self.setup_paths() [cite: 2]
        self.setup_logging() [cite: 2]
        
        # Load interval from config, default to 500
        self.cache_save_interval = self.config.get("cache_save_interval", 500) [cite: 2]
    ...
    def setup_paths(self):
        self.xls_path = self.config.get('xls_path') [cite: 4]
        self.archive_name = self.config.get('archive_folder_name') [cite: 4]
        # Pulls the export path from config or defaults to a local filename
        self.cache_export_path = self.config.get('cache_export_path', 'SMTP_Cache_Export.xlsx')

    def setup_logging(self):
        self.bulk_logger = self._create_logger('bulk_logger', self.config.get('log_bulk_path')) [cite: 4]
        self.invalid_logger = self._create_logger('invalid_logger', self.config.get('log_invalid_path')) [cite: 4]
    ...
    def _create_logger(self, name, log_file):
        ...
        if not logger.handlers:
            ...
            handler.setFormatter(formatter) [cite: 5]
    ...
    def load_data(self):
        ...
        try:
            with pd.ExcelFile(self.xls_path) as xls: [cite: 6]
                sheet_map = self.config.get('sheet_map', {})
                
                for rule_name, info in sheet_map.items():
                    df = pd.read_excel(xls, info['sheet']) [cite: 6]
                    dest = info['destination_name'] [cite: 6]
                    
                    # Logic for Email rules
                    if 'Email' in rule_name and 'Keyword' not in rule_name: [cite: 7]
                        col = info['column'] [cite: 7]
                        addresses = df[col].dropna().unique().tolist() [cite: 7]
                        for addr in addresses: [cite: 8]
                            # Rule 8 specific: ResearchEmail is sender only
                            is_sender_only = (rule_name == "ResearchEmail") [cite: 8]
                            self.email_rules[addr.lower()] = {"dest": dest, "sender_only": is_sender_only} [cite: 9]
                    
                    # Logic for Keyword rules
                    else:
                        cols = info.get('columns', [info.get('column')]) [cite: 10]
                        match_field = info.get('match_field', 'subject_only') [cite: 10]
                        for col in cols:
                            keywords = df[col].dropna().unique().tolist() [cite: 11]
                            for kw in keywords:
                                self.keyword_rules[str(kw).lower()] = {"dest": dest, "field": match_field}

                # Load SMTP Cache
                try: [cite: 12]
                    cache_df = pd.read_excel(xls, 'SMTP_Cache') [cite: 12]
                    self.smtp_cache = dict(zip(cache_df['ExchangeAddress'].str.lower(), cache_df['SMTPAddress'])) [cite: 12]
                except:
                    self.smtp_cache = {}
        except Exception as e:
            self.invalid_logger.critical(f"DataLoaderError||{e}") [cite: 13]

    def save_smtp_cache(self):
        """Saves cache back to the specified export file."""
        try:
            df = pd.DataFrame(list(self.smtp_cache.items()), columns=['ExchangeAddress', 'SMTPAddress']) [cite: 14]
            # Uses the path defined in setup_paths
            with pd.ExcelWriter(self.cache_export_path, engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name='SMTP_Cache', index=False)
            self.items_since_last_save = 0 [cite: 14]
        except Exception as e:
            self.invalid_logger.error(f"CacheSaveError||{e}") [cite: 15]
        ...
        try:
            ...
            if sender_obj.AddressEntryUserType == 0: # olExchangeUserAddressEntry
                ...
                if ex_addr in self.smtp_cache:
                    return self.smtp_cache[ex_addr] [cite: 16]
    ...
    def process_email(self, item, archive_root):
        ...
        try:
            subject = str(item.Subject).lower() [cite: 19]
            body = str(item.Body).lower() [cite: 19]
            sender_email = (self.get_smtp_address(item) or "").lower() [cite: 19]
            
            # 1. Check Email Rules
            if sender_email in self.email_rules:
                rule_info = self.email_rules[sender_email] [cite: 20]
            ...
            for kw, info in self.keyword_rules.items(): [cite: 21]
                match = False
    ...
    def execute_action(self, item, dest_name, archive_root, trigger, match_type):
        ...
        try:
            if dest_name == "ToDelete":
                item.Delete()
                self.bulk_logger.info(f"DELETED|{trigger}|{match_type}|{item.Subject}") [cite: 24]
                return True
            else:
    ...
    def run_archive_processing(self, target_folder_path):
        ...
        try:
            ...
            if target_folder_path != "ROOT":
                for part in target_folder_path.split('\\'): [cite: 28]
    ...
    def _process_folder_items(self, folder, archive_root):
        ...
        try:
            items = folder.Items
            count = items.Count [cite: 30]
            for i in range(count, 0, -1):
                try:
                    item = items.Item(i)
                    if item.Class == self.MAIL_ITEM_CLASS:
                        if self.process_email(item, archive_root): [cite: 31]
                            self.processed_count += 1
                            self.items_since_last_save += 1
                        
                        if self.items_since_last_save >= self.cache_save_interval: [cite: 32]
                            self.save_smtp_cache()
                except Exception as e:
                    # Likely item moved/deleted or COM error [cite: 33]
                    continue
        except Exception as e:
    ...
    def start_gui(self):
        ...
        selected_folder = tk.StringVar(value="Inbox") [cite: 35]
        for f in folders:
        ...
        def start_task():
            ...
            if messagebox.askyesno("Confirm", f"Process matching emails in {folder_to_process}?\n\n'ToDelete' items will be PERMANENTLY DELETED."):
                btn_start.config(state=tk.DISABLED) [cite: 36]
