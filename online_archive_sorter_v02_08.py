...
class OnlineArchiveSorter:
    ...
    def setup_logging(self):
        # Create loggers with UTF-8 encoding
    ...
    def _create_logger(self, name, log_file):
        """Create a logger with UTF-8 encoding support"""
        ...
        try:
            ...
            if log_file:
                ...
                # Create file handler with UTF-8 encoding - THIS IS THE KEY FIX
                file_handler = logging.FileHandler(log_file, encoding='utf-8')
                formatter = logging.Formatter('%(asctime)s|%(levelname)s|%(message)s')
                file_handler.setFormatter(formatter)
                logger.addHandler(file_handler)
                
                print(f"Created logger '{name}' with UTF-8 encoding for file: {log_file}")
            
        except Exception as e:
            print(f"Warning: Could not setup logger {name} with UTF-8: {e}")
    ...
    def execute_action(self, item, dest_name, archive_root, trigger, match_type):
        ...
        try:
            if dest_name == "ToDelete":
                item.Delete()
                # Use safe logging for console output
                safe_subject = self._safe_string(str(item.Subject))
                print(f"DELETED|{trigger}|{match_type}|{safe_subject}")
                ...
                # Use safe logging for console output
                safe_subject = self._safe_string(str(item.Subject))
                print(f"MOVED|{dest_name}|{trigger}|{match_type}|{safe_subject}")
    ...
    def _safe_string(self, text):
        """Helper method to safely handle Unicode strings for console output"""
        try:
            return text
        except:
            # If there's an issue, replace non-ASCII characters
